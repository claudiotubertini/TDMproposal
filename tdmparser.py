...
class TDMFileParser:
    ...
    def __init__(self, url=''):
        ...
        base_url = urllib.parse.urlunparse(parsed._replace(path='', params='', query='', fragment=''))
        self.jsonfile = urllib.parse.urljoin(base_url, '/.well-known/tdmrep.json')
        self.last_checked = 0
        self.allow_all = True
    ...
    def set_url(self, url):
        """Sets the URL to be checked referring to a tdmrep.json file."""
        self.url = url
    ...
    def read(self):
        ...
        try:
            f = urllib.request.urlopen(self.jsonfile)
...
class TDMHeader:
    ...
    def __init__(self, path):
        self.path = urllib.parse.urlunparse(urllib.parse.urlparse(path))
    ...
    def is_allowed(self):
        ...
        try:
            if ret['tdm-reservation'] == '0':
                logging.debug('tdm-reservation = 0')
                return True
            elif ret['tdm-reservation'] == '1':
                if ret['tdm-policy']:
                    logging.debug(f'tdm-reservation = 1; tdm-policy: {ret["tdm-policy"]}')
                    return ret['tdm-policy']
                else:
                    logging.debug('tdm-reservation = 1 and any policy gas been specified')
    ...
    def __str__(self):
        if self.is_allowed():
            ...
            policy = self._get_headers()['tdm-policy']
            if policy:
               ret = ret + f' but you can read a policy: {policy}.'
        else:
            ret = 'This content appears to be freely mined, but check again.'
        return ret
...
class TDMhtmlHead:
    ...
    def is_allowed(self):
        ...
        try:
            ...
            logging.debug('No tdm-reservation has been specified')
            return True ####### TODO
    ...
    def __str__(self):
        if self.is_allowed():
            ...
            ret = self.path + ' cannot be freely mined'
            policy = self._get_head()['tdm-policy']
            if policy:
               ret = ret + f' but you can read a policy: {policy}.'
        else:
            ret = 'This content appears to be freely mined, but check again.'
    ...
    def allowance(self, filename):
        ...
        if m.match(filename):
            ...
            if self.tdm_reservation == 1 and self.tdm_policy:
                logging.debug(f'tdm-reservation = 1; tdm-policy: {self.tdm_policy}')
            ...
            logging.debug('No tdm-reservation has been specified but there could be a problem with the url searched')
